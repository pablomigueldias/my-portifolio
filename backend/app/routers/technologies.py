...
@router.delete("/{tech_id}/", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(validate_admin)])
def delete_technology(tech_id: int, db: Session = Depends(get_db)):
    service = TechnologyService(db)
    success = service.delete_technology(tech_id)
    if not success:
        raise HTTPException(
            status_code=404, detail="Tecnologia não encontrada")
    return None
